from src.Park.Visitor import Visitor
from src.Park.Attraction import Attraction
from src.Park.Ticket import Ticket
from src.Park.Exception import QueueEmptyError
from src.Park.SafetyRequirements import SafetyRequirements
...
class Park:
    ...
    @property
    def visitors_list(self) -> list[Visitor]:
        return list(self._visitors.values())
        
    @property
    def attractions_list(self) -> list[Attraction]:
        return list(self._attractions.values())

    # В MVC Модель получает данные через параметры, а не через input()
    def create_visitor(self, name: str, firstname: str, lastname: str, wallet: int) -> int:
    ...
    def create_attraction(self, name: str, atype: str) -> int:
    ...
    def add_visitor_to_attraction(self, vid: int, aid: int) -> None:
        ...
        if visitor.transaction(ticket):
            ...
            print("something went wrong / no money")
